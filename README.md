...
high or low. Of course the Vcc and Ground pins can all be wired on
...
I'll show two simple controllers later: they're done as 
very low-end and cheap implementations. Probably not 
the right thing for your concert in Hyde Park,
but good enough for home use. 
...
I've done a really simple controllers: 
very low-end and cheap. Probably not 
the right thing for playing your concert in Hyde Park,
but good enough for home use. 
...
Sure enough, I've managed to turn my guitar amplifier into  
a rather expensive blinking LED.    
...
Each switch on the Boss FS-6 foot switch can be selected to 
be either instantaneous or latching. 
...
amp, and a class for handling an 
input (a switch, a mouse button, 
a touch controller, etc). and the main driver 
loop, which must instantiate objects for each 
switch input, each output line, and must perform
the cross-connect and move events from the switches
to the line drivers.
...
I bought a handful of TTP223-based proximity touch sensor 
switches from Banggood, typically about $9 for 20 switches.  
...
a common bus.  So an 8-core cable is required from the board
back to the ESP32: six switch outputs, Vcc, and Ground. Ethernet 
cable has 8-cores, and I had some lying around. So here is the 
wiring harness being put in place:  
![rearWiring](images/rear_wiring.jpg "Rear wiring")
